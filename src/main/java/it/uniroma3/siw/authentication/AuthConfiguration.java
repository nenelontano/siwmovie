...
public class AuthConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http
				...
				.antMatchers(HttpMethod.GET, "/", "/index", "/login", "/register", "/css/**","/bootstrap/**", "/images/**", "favicon.ico","/artist","/movie/**","/artist/**", "/movies","/movies/**", "/formSearchMovies", "/formSearchArtists").permitAll()
				// chiunque (autenticato o no) può mandare richieste POST al punto di accesso per login e register
				.antMatchers(HttpMethod.POST, "/login", "/register","/artist","/movie/**","/artist/**","/movies/**", "/searchMovies", "/searchArtists").permitAll()
				// solo gli utenti autenticati con ruolo ADMIN possono accedere a risorse con path /admin/**
				.antMatchers(HttpMethod.GET, "/admin/**").hasAnyAuthority(ADMIN_ROLE)
				.antMatchers(HttpMethod.POST, "/admin/**").hasAnyAuthority(ADMIN_ROLE)
				.antMatchers(HttpMethod.GET, "/user/**").hasAnyAuthority(DEFAULT_ROLE,ADMIN_ROLE)
				.antMatchers(HttpMethod.POST, "/user/**").hasAnyAuthority(DEFAULT_ROLE,ADMIN_ROLE)
